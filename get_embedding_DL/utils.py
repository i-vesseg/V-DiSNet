...
def get_dataset(dataset_name, datadir, train, im_size, patch_size, patch_type='random'):
    ...
    if dataset_name not in ['codes']:
        transforms.append(Resize(im_size)) # Resize
        if patch_size > 0:
    ...
    if dataset_name == 'MNIST':
        ...
        transforms.append(ToTensor())
        try:
            MEAN_VESSEL, STD_VESSEL = np.load(os.path.join(datadir, 'VESSEL_mean_std.npy'))
        except FileNotFoundError:
            MEAN_VESSEL, STD_VESSEL = np.load('/home/falcetta/0_PhD/sparse_var/deep-sparse/data/VESSEL/VESSEL_mean_std.npy')
        print(MEAN_VESSEL, STD_VESSEL) # 0.07933422649793388 0.27025970288577694
            
        transforms.append(Normalize(mean=MEAN_VESSEL, std=STD_VESSEL))
    else:
        raise NotImplementedError
    ...
    if dataset_name == 'MNIST':
        ...
        print(f"Dowloading MNIST dataset to {datadir}...")
        ...
        split = 'train' if train else 'test'
        print(f"Reading VESSEL dataset from {datadir}/{split}...")
        try:
            dataset = datasets_torch.ImageFolder(root=f'{datadir}/{split}', transform=transforms)
        except FileNotFoundError:
            print(f"WARNING: NO SPLIT FOLDER!! Reading VESSEL dataset from {datadir}...")
            dataset = datasets_torch.ImageFolder(root=datadir, transform=transforms)
    else:
        raise NotImplementedError
    return dataset
...
def inverse_transform(X, dataset_name):
    if dataset_name == 'MNIST':
        ...
        datadir = '/home/falcetta/0_PhD/sparse_var/deep-sparse/data/VESSEL' # NON MI PIACE MA PER ORA VA BENE
        mean, std = np.load(os.path.join(datadir, 'VESSEL_mean_std.npy'))
    elif dataset_name == 'VPATCHES':
        datadir = '/home/falcetta/0_PhD/sparse_var/deep-sparse/data/VESSEL' # NON MI PIACE MA PER ORA VA BENE
        mean, std = np.load(os.path.join(datadir, 'VESSEL_mean_std.npy'))
        #print(mean, std)
    elif dataset_name == 'VPATCHES_TRUE':
        metadata= np.load('/data/falcetta/datasets/preprocess_OASIXI/patches_preprocessed/dataloader_metadata/metadata.npz')
...
def binarize_prediction(target, pred):
    #normalize to 0,255
    assert(), "BINARY NORM"
    target = (target - target.min()) / (target.max() - target.min()) * 255
    pred = (pred - pred.min()) / (pred.max() - pred.min()) * 255
    assert target.max() == 255 and target.min() == 0
    assert pred.max() == 255 and pred.min() == 0
    #binarize pred using a threshold of 0.5
    pred = torch.where(pred > 100, 255, 0)
...
def my_iterator(args, data, log_interval=10):
    for epoch in tqdm(range(args.epochs), desc='Epochs'):
        for batch_id, (X, extra) in enumerate(tqdm(data, desc='Batches', leave=False)):
            #print(f"Batch {batch_id} of {len(data)}")
            batch = {}
            batch['batch_id'] = batch_id
            batch['X'] = X # X is the input
            if args.dataset == 'imagenet_LCN':
                batch['extra'] = extra
            if args.dataset == 'MNIST':
                batch['target'] = extra # extra is the target (label)
            if args.dataset == 'VESSEL':
                #print("AAAAAAAAAAAAAAAAAAA")
                pass
            if args.dataset == 'VPATCHES':
                batch['X']= extra # Use the extra as the input 
                #normalize
                mean, std = 0.07933422649793388, 0.27025970288577694
                batch['X'] = (batch['X'] - mean) / std
            if args.dataset == 'VPATCHES_TRUE':
                batch['X']= X
                #normalize
                metadata= np.load('/data/falcetta/datasets/preprocess_OASIXI/patches_preprocessed/dataloader_metadata/metadata.npz')
                mean, std = metadata['mean'], metadata['std']
            ...
            # batch['X'] shape = (batch_size, 1, 32, 32) if VPATCHES else (batch_size, 1, 32, 32)!!!!!!!!!!!!!
...
def log_viz(decoder, writer, n_samples, y, y_hat, Zs, stats,
    ...
    # Log target
    #print(f"Saving viz in {img_dir} --- {viz_type}")
    ...
    if log_all:
        ...
        #print("Saving decoder columns 2")
