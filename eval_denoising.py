...
from utils import get_dataset, mk_dir, set_random_seed, FixedSubsetSampler, my_iterator_val, np, \
...
def eval_denoising(args):
    ...
    # Working directory
    outdir = lambda dirname: os.path.join(args.outdir, dirname, args.date_exp)
    ...
    if args.dataset == 'VPATCHES':
        ...
        path_data = "/data/falcetta/datasets"
        original_data_dir = os.path.join(path_data, "preprocess_OASIXI/test")
        preprocessed_data_dir = os.path.join(path_data, "preprocess_OASIXI/patches_preprocessed")
        ...
        dataloader_parameters = {
            ...
            "validation_split": 1, # Just to get the test set
        ...
        _ , data_eval = get_dataloader(preprocessed_data_dir, standardize_volume, use_true_labels_train, original_data_dir, **dataloader_parameters)
        args.n_channels = 1
    else:
        # Evaluation data
    ...
    # Load decoder
    print(f"Loading decoder from {args.pretrained_path_dec}")
    ...
    # Load encoder
    print(f"Loading encoder from {args.pretrained_path_enc}")
    ...
    for batch, batch_info, should in my_iterator_val(args, data_eval, log_viz_interval):
        ...
        psnr_noisy_img = PSNR(y, y_noisy, args.dataset,
                              ...
                              pred_sample_mean=y_noisy_mean, pred_sample_std=y_noisy_std, binary_output=args.binary_output)
        ...
        psnr_orig = PSNR(y, y_hat_orig, args.dataset,
                         tar_sample_mean=y_orig_mean, tar_sample_std=y_orig_std, binary_output=args.binary_output)
        ...
        img_eval_denos = os.path.join(img_dir, 'eval_denoising')
        mk_dir(img_eval_denos)
        ...
        if should['log_val_imgs']:
            ...
            #print("------ Saving images (y, y_noisy, y_hat_orig, y_hat_noisy) ------")
            save_img(y, f'{img_eval_denos}/y_{(batch_id+1):04d}.png', norm=False)
            save_img(y_noisy, f'{img_eval_denos}/y_noisy_{(batch_id+1):04d}.png', norm=False)
            save_img(y_hat_orig, f'{img_eval_denos}/y_hat_orig_{(batch_id+1):04d}.png', norm=False)
            save_img(y_hat_noisy, f'{img_eval_denos}/y_hat_noisy_{(batch_id+1):04d}.png', norm=False)
    ...
    save_img(cols, f'{img_eval_denos}/lin_dec_cols.png', norm=False, n_rows=int(2 ** (np.log2(n_cols) // 2)))
